...
static Channel piano_chan_alloc(char *song) {
  NoteReadState nrs = { .str = song };

  int note_len = 0;
  while (tune_parse(&nrs))
    if (nrs.ret.kind != NrsRetKind_None) note_len++;

  i2sNote *notes = (i2sNote *)calloc(sizeof(i2sNote), note_len);
  if (notes == NULL) {
    printf("couldn't alloc space for %d notes!\n", note_len);
    return (Channel) {0};
  }

  nrs = (NoteReadState) { .str = song };
  int i = 0;
  while (tune_parse(&nrs)) {
    if (nrs.ret.kind == NrsRetKind_None)
      continue;
    if (nrs.ret.kind == NrsRetKind_Sound)
      notes[i++] = i2snote_sound(nrs.ret.sound.freq ,
                                 nrs.ret.duration   ,
                                 nrs.ret.sound.sound);
    if (nrs.ret.kind == NrsRetKind_Pause)
      notes[i++] = i2snote_pause(nrs.ret.duration);
  }
...
void piano_init(void) {
  ...
  for (int i = 0; i < TABLE_LEN; i++) {
    ...
    piano_state.sample_table[Sound_Square  ][i] = 16384 * ((fmodf(t, 0.5f) > 0.25f) ? 1.0f : -1.0f);
  }
...
static int16_t piano_compute_sample(Channel *chan) {
  ...
  int ni = chan->note_i;
  while (ni < chan->note_len && chan->notes[ni].step) ni++;
  if (ni < chan->note_len) ni += 1;

  int ret = 0;
  for (int i = chan->note_i; i < ni; i++) {
    i2sNote *note = chan->notes + i;

    if (note->step == 0)
      continue;
    else {
      ret += (40 * piano_state.sample_table[note->sound][chan->pos >> 16u]) >> 8u;
      chan->pos += note->step;
  ...
  if (chan->samples_into_note >= chan->notes[chan->note_i].sample_duration) {
    chan->samples_into_note = 0;

    /* no further to go? we're done */
    if (ni == chan->note_i) {
