...
const {
  ...
  exports.addText = (str, opts = {}) => {
    ...
    native.text_add(str, opts.color ?? "1", opts.x ?? padLeft, opts.y ?? 0);
  };
  ...
  exports.setPushables = (pushTable) => {
    ...
    for (const [pusher, pushesList] of Object.entries(pushTable))
      for (const pushes of pushesList) native.push_table_set(pusher, pushes);
  };

  let afterInputs = [];
  exports.afterInput = (fn) => (
    console.log("engine.js:afterInputs"), afterInputs.push(fn)
  );
...

  {
    ...
    setInterval = (fn, ms = 10) => (
      (timers[id] = { fn, ms, restartAt: ms }), id++
    );
  ...
  function _makeTag(cb) {
    return (strings, ...interps) => {
      if (typeof strings === "string") {
        throw new Error(
          "Tagged template literal must be used like name`text`, instead of name(`text`)"
        );
      }
      const string = strings.reduce(
        (p, c, i) => p + c + (interps[i] ?? ""),
        ""
      );
      return cb(string);
